...
def policy_evaluation(
    ...
    while True:
        ...
        for s in range(nS):
            ...
            a = pi[s]
            V_new[s] = R_sa[s, a] + gamma * np.sum(T[s, a, :] * V)

        delta = np.max(np.abs(V_new - V))
        V = V_new
        # stop if values change only a tiny bit
        if delta < epsilon:
            break
...
def policy_improvement(
    ...
    for s in range(nS):
        for a in range(nA):
            # Q(s,a) = immediate reward + discounted expected future value
            Q[s, a] = R_sa[s, a] + gamma * np.sum(T[s, a, :] * V)
    # choose the best action in each state
